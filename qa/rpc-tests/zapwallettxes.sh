...
if [ $# -lt 1 ]; then
        ...
        echo "e.g. $0 ../../src"
        echo "Env vars QTCOIND and QTCOINCLI may be used to specify the exact binaries used"
        exit 1
fi

set -f

QTCOIND=${QTCOIND:-${1}/qtcoind}
CLI=${QTCOINCLI:-${1}/qtcoin-cli}
...
$QTCOIND -zapwallettxes=1 $B1ARGS &
B1PID=$!
$QTCOIND -zapwallettxes=2 $B2ARGS &
B2PID=$!
