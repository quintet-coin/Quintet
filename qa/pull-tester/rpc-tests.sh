...
export QTCOINCLI=${BUILDDIR}/qa/pull-tester/run-qtcoin-cli
export QTCOIND=${REAL_QTCOIND}
...
if [ "x${ENABLE_QTCOIND}${ENABLE_UTILS}${ENABLE_WALLET}" = "x111" ]; then
  ...
  echo "No rpc tests to run. Wallet, utils, and qtcoind must all be enabled"
fi
