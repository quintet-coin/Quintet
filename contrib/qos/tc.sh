...
LINKCEIL="1gbit"
#limit outbound QuinTet protocol traffic to this rate
LIMIT="160kbit"
...
if [ ! -z "${LOCALNET_V6}" ] ; then
	ip6tables -t mangle -A OUTPUT -p tcp -m tcp --dport 9222 ! -d ${LOCALNET_V6} -j MARK --set-mark 0x4
	ip6tables -t mangle -A OUTPUT -p tcp -m tcp --sport 9222 ! -d ${LOCALNET_V6} -j MARK --set-mark 0x4
fi
