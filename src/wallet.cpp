...
/**
 * Fees smaller than this (in uqtc) are considered zero fee (for transaction creation)
 * We are ~100 times smaller then qtcoin now (2015-06-23), set minTxFee 10 times higher
 * so it's still 10 times lower comparing to qtcoin.
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 QTC.");
                    }
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-qtc-address
                    CScript scriptChange;
